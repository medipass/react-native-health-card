import React from 'react';
import PropTypes from 'prop-types';
import { Image, Text, View } from 'react-native';
import FlipCard from 'react-native-flip-card';

import commonStyles from '../../common.styles';
import genericCardStyles from './styles/genericCard.styles';
import { getFocusStyle } from '../../utils/textFocus';
import PositionableField from './positionable_field';
import BackgroundImages from './BackgroundImages';

/*eslint-disable*/
const defaultLogo = 'data:image/png;base64,';
/*eslint-enable*/

export const GenericCard = (props) => {
    const backAttributes = [];
    if (props.memberNumberPosition === 'back') {
        backAttributes.push('memberNumber');
    }
    if (props.rankPosition === 'back') {
        backAttributes.push('rank');
    }
    if (props.issueNumberPosition === 'back') {
        backAttributes.push('issueNumber');
    }
    const logo = props.logo && props.logo.length > 0 ? props.logo : defaultLogo;
    const showBackgroundImage = props.applyBackgroundImage && BackgroundImages[props.type];
    const cardStyleBackground = showBackgroundImage ? { backgroundColor: null } : {};
    return (
        <FlipCard
            style={[props.cardStyle, cardStyleBackground]}
            friction={6}
            perspective={1000}
            flipVertical={false}
            flip={props.showBack || backAttributes.includes(props.focus)}
            flipHorizontal
            clickable
        >
            <View style={[commonStyles.front, { width: props.width, height: props.height }]}>
                {
                    showBackgroundImage ?
                        <Image source={{ uri: BackgroundImages[props.type].front }} style={[commonStyles.bgImage, { width: props.width, height: props.height }]} />
                        :
                        <View style={[commonStyles.row]}>
                            <Text style={[commonStyles.text, genericCardStyles.healthFundTitle]}>{props.title}</Text>
                            <Image source={{ uri: logo }} resizeMode={'contain'} style={[genericCardStyles.logo, props.logoStyle]} />
                        </View>
                }
                {
                    props.memberNumberPosition === 'front' &&
                    <PositionableField
                        isHighlighted={props.focus === 'memberNumber'}
                        fieldText1={props.memberNumberText}
                        fieldText2={props.memberNumber}
                        fieldStyle={[props.memberNumberFieldStyle]}
                        fieldTextStyle={[commonStyles.text, commonStyles.title, { paddingHorizontal: 5 }, props.memberNumberTextStyle]}
                        fieldTextHighlightStyle={[commonStyles.text, commonStyles.title, { paddingHorizontal: 5 }, props.memberNumberTextHighlightStyle]}
                    />
                }
                {
                    props.rankPosition === 'front' &&
                    <PositionableField
                        isHighlighted={props.focus === 'rank'}
                        fieldText1={props.rank}
                        fieldText2={props.cardHolderName}
                        fieldStyle={[props.rankFieldStyle]}
                        fieldTextStyle={[commonStyles.text, commonStyles.title, { paddingHorizontal: 5 }, props.rankTextStyle]}
                        fieldTextHighlightStyle={[commonStyles.text, commonStyles.title, { paddingHorizontal: 5 }, props.rankTextHighlightStyle]}
                    />
                }
                {
                    props.issueNumberPosition === 'front' &&
                    <PositionableField
                        isHighlighted={props.focus === 'issueNumber'}
                        fieldText1={props.issueNumberText}
                        fieldText2={props.issueNumber}
                        fieldStyle={[props.issueNumberFieldStyle]}
                        fieldTextStyle={[commonStyles.text, commonStyles.title, { paddingHorizontal: 5 }, props.issueNumberTextStyle]}
                        fieldTextHighlightStyle={[commonStyles.text, commonStyles.title, { paddingHorizontal: 5 }, props.issueNumberTextHighlightStyle]}
                    />
                }
            </View>
            <View style={[commonStyles.back, { width: props.width, height: props.height }]}>
                {
                    showBackgroundImage &&
                    <Image source={{ uri: BackgroundImages[props.type].back }} style={[commonStyles.bgImage, { width: props.width, height: props.height }]} />
                }
                {
                    props.showSwipeBar && <View style={commonStyles.bar} />
                }
                {
                    props.memberNumberPosition === 'back' &&
                    <PositionableField
                        isHighlighted={props.focus === 'memberNumber'}
                        fieldText1={props.memberNumberText}
                        fieldText2={props.memberNumber}
                        fieldStyle={[props.memberNumberFieldStyle]}
                        fieldTextStyle={[commonStyles.text, commonStyles.title, { paddingHorizontal: 5 }, props.memberNumberTextStyle]}
                        fieldTextHighlightStyle={[commonStyles.text, commonStyles.title, { paddingHorizontal: 5 }, props.memberNumberTextHighlightStyle]}
                    />
                }
                {
                    props.rankPosition === 'back' &&
                    <PositionableField
                        isHighlighted={props.focus === 'rank'}
                        fieldText1={props.rank}
                        fieldText2={props.cardHolderName}
                        fieldStyle={[props.rankFieldStyle]}
                        fieldTextStyle={[commonStyles.text, commonStyles.title, { paddingHorizontal: 5 }, props.rankTextStyle]}
                        fieldTextHighlightStyle={[commonStyles.text, commonStyles.title, { paddingHorizontal: 5 }, props.rankTextHighlightStyle]}
                    />
                }
                {
                    props.issueNumberPosition === 'back' &&
                    <PositionableField
                        isHighlighted={props.focus === 'issueNumber'}
                        fieldText1={props.issueNumberText}
                        fieldText2={props.issueNumber}
                        fieldStyle={[props.issueNumberFieldStyle]}
                        fieldTextStyle={[commonStyles.text, commonStyles.title, { paddingHorizontal: 5 }, props.issueNumberTextStyle]}
                        fieldTextHighlightStyle={[commonStyles.text, commonStyles.title, { paddingHorizontal: 5 }, props.issueNumberTextHighlightStyle]}
                    />
                }
            </View>
        </FlipCard>
    );
}

GenericCard.propTypes = {
    title: PropTypes.string,
    type: PropTypes.string,
    logo: PropTypes.string,
    logoStyle: PropTypes.object,
    cardHolderName: PropTypes.string,
    cardStyle: PropTypes.array,
    showSwipeBar: PropTypes.bool,
    width: PropTypes.number,
    height: PropTypes.number,
    focus: PropTypes.string,
    issueDate: PropTypes.string,
    issueNumberPosition: PropTypes.string,
    issueDateText: PropTypes.string,
    issueNumber: PropTypes.string,
    issueNumberText: PropTypes.string,
    memberNumber: PropTypes.string,
    memberNumberText: PropTypes.string,
    rank: PropTypes.string,
    rankText: PropTypes.string,
    showCardHolderName: PropTypes.bool,
    showMembershipNumber: PropTypes.bool,
    showIssueDate: PropTypes.bool,
    showRank: PropTypes.bool,
    showIssueNumber: PropTypes.bool,
    showBack: PropTypes.bool,
    applyBackgroundImage: PropTypes.bool,
};

GenericCard.defaultProps = {
    title: 'HEALTH FUND',
    logo: defaultLogo,
    logoStyle: {},
};

export default GenericCard;
